...
class TaxRateBoServiceSpec  extends Specification {
  ...
  "return product tax rate" in {
    ...
    val country = "FR"
    val state = None
    val product = new Product(id=47,name="",xtype=ProductType.PRODUCT, calendarType=ProductCalendar.NO_DATE,taxRate=Some(defaultTaxRate),shippingFk=None,startDate=None,stopDate=None)
