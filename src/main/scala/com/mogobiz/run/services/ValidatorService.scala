...
class ValidatorService(implicit settings: RoutingSettings, refFactory: ActorRefFactory)
    extends Directives
  ...
  val route = {
    path(Segment / "download" / Segment) { (storeCode, key) =>
      get {
        handleCall(validatorHandler.download(storeCode, key), (res: (String, File)) => {
          val mediaTypeOpt =
            MediaType.custom(MimeTypeTools.detectMimeType(res._2).getOrElse("application/octet-stream"))
