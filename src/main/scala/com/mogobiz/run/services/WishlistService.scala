...
class WishlistService extends Directives with DefaultComplete {
  ...
  def addItem(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "item") {
  ...
  def removeItem(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "item" / Segment) {
  ...
  def addIdea(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "idea") {
    (wishlist_list_uuid, wishlist_uuid) =>
      post {
        parameters('idea, 'owner_email) { (idea, owner_email) =>
          handleCall(wishlistHandler.addIdea(storeCode,
                                             wishlist_list_uuid,
                                             wishlist_uuid,
  ...
  def removeIdea(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "idea" / Segment) {
    ...
    (wishlist_list_uuid, wishlist_uuid) =>
      post {
        entity(as[AddBrandCommand]) { cmd =>
          handleCall(wishlistHandler.addBrand(storeCode,
                                              wishlist_list_uuid,
                                              wishlist_uuid,
                                              WishBrand(GlobalUtil.newUUID, cmd.name, cmd.brand),
                                              cmd.owner_email),
                     (id: String) => complete(StatusCodes.OK, id))
        }
      }
  ...
  def removeBrand(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "brand" / Segment) {
    (wishlist_list_uuid, wishlist_uuid, brand_uuid) =>
      delete {
        parameters('owner_email) { (owner_email) =>
          handleCall(
              wishlistHandler.removeBrand(storeCode, wishlist_list_uuid, wishlist_uuid, brand_uuid, owner_email),
              (x: Unit) => complete(StatusCodes.OK))
        }
      }
  }
    ...
    (wishlist_list_uuid, wishlist_uuid) =>
      post {
        entity(as[AddCategoryCommand]) { cmd =>
          handleCall(wishlistHandler.addCategory(storeCode,
                                                 wishlist_list_uuid,
                                                 wishlist_uuid,
                                                 WishCategory(GlobalUtil.newUUID, cmd.name, cmd.category),
                                                 cmd.owner_email),
                     (id: String) => complete(StatusCodes.OK, id))
        }
      }
  ...
  def removeCategory(implicit storeCode: String) = path(Segment / "wishlist" / Segment / "category" / Segment) {
    (wishlist_list_uuid, wishlist_uuid, category_uuid) =>
      delete {
        parameters('owner_email) { (owner_email) =>
          handleCall(
              wishlistHandler.removeCategory(storeCode, wishlist_list_uuid, wishlist_uuid, category_uuid, owner_email),
              (x: Unit) => complete(StatusCodes.OK))
        }
      }
  }
   ...
   */
  def setOwnerInfo(implicit storeCode: String) = path(Segment / "owner") { wishlist_list_uuid =>
    post {
      parameters('owner_email,
                 'owner_name.?,
                 'owner_day_of_birth.?.as[Option[Int]],
                 'owner_month_of_birth.?.as[Option[Int]],
                 'owner_description.?) {
        (owner_email, owner_name, owner_day_of_birth, owner_month_of_birth, owner_description) =>
          handleCall(wishlistHandler.setOwnerInfo(storeCode,
                                                  wishlist_list_uuid,
  ...
  def addWishlist(implicit storeCode: String) = path(Segment / "wishlist") { wishlist_list_uuid =>
    post {
  ...
  def getWishlistByToken(implicit storeCode: String) = path("wishlist" / Segment) { token =>
    get {
