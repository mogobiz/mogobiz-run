...
object NotationDAO extends SQLSyntaxSupport[Notation] with SqlService {
  ...
  def apply(rn: ResultName[Notation])(rs: WrappedResultSet): Notation =
    Notation(rs.get(rn.id),
  ...
  def create(notation: com.mogobiz.run.model.Notation)(implicit session: DBSession): Notation = {
    val newNotation =
      new Notation(newId(), UUID.fromString(notation.uuid), JacksonConverter.serialize(notation), new Date, new Date)
    ...
    DB localTx { implicit session =>
      applyUpdate {
        QueryDSL
          ...
          .where
          .eq(NotationDAO.column.uuid, notation.uuid)
      }
    }
  }
}
