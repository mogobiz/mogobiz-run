  ...
  def querySkusByCriteria(storeCode: String, skuRequest: SkuRequest): JValue = {
    ...
    val filters: List[FilterDefinition] = List(
        ...
        skuRequest.countryCode.map { countryCode =>
          termFilter(s"${countryCode}.enabled", true)
        },
        ...
        createOrFilterBySplitValues(skuRequest.tagName.map(_.toLowerCase), v =>
              createNestedTermFilter("product.tags", "product.tags.name", Some(v))),
...
//        println("newStock = ",newStock )
  ...
  def existsAvailableSkus(indexEs: String, productId: Long): Boolean = {
    ...
    val filters: List[FilterDefinition] =
      List(createTermFilter("product.id", Some(productId)).get, createTermFilter("available", Some(true)).get)
