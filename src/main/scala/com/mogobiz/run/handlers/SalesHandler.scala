...
class SalesHandler {
  ...
  def incrementSales(indexEs: String, product: Product, sku: Sku, quantity: Long)(implicit session: DBSession =
                                                                                    AutoSession): SaleChange = {
    val nbProductSales =
      sql"select nb_sales from product where id=${product.id}".map(rs => rs.long("nb_sales")).single().apply().get
    val newNbProductSales = nbProductSales + quantity
    sql"update product set nb_sales = ${newNbProductSales},last_updated = ${DateTime.now} where id=${product.id}"
      .update()
      .apply()

    val nbSkuSales =
      sql"select nb_sales from ticket_type where id=${sku.id}".map(rs => rs.long("nb_sales")).single().apply().get
    val newNbSkuSales = nbSkuSales + quantity
    sql"update ticket_type set nb_sales = ${newNbSkuSales},last_updated = ${DateTime.now} where id=${sku.id}"
      .update()
      .apply()
