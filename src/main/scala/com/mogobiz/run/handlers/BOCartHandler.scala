...
class BOCartHandler extends BoService {
  ...
  def findByTransactionUuid(storeCode: String,
                            transactionUuid: String): Option[BOCart] = {
    val query = search(buildIndex(storeCode) -> "BOCart") query must(
      matchQuery("transactionUuid", transactionUuid))
    EsClient.search[BOCart](query)
  }

  def esSave(storeCode: String,
             boCart: BOCart,
  ...
  def initRegisteredCartItem(
    ...
    BORegisteredCartItem(
      age = Utils.computeAge(registeredCartItem.birthdate),
  ...
  def initBOShopCart(shopCart: StoreShopCartWithPrices,
                     rate: Currency,
                     cartItems: List[BOCartItem]): BOShopCart = {
  ...
  def initBOCart(companyFk: Long,
                 uuid: String,
                 buyer: String,
                 cart: StoreCartWithPrices,
                 shippingAddress: AccountAddress,
                 shopCarts: List[BOShopCart]): BOCart = {
    BOCart(
      uuid,
...
case class BODelivery(
    @JsonScalaEnumeration(classOf[DeliveryStatusRef]) status: DeliveryStatus.DeliveryStatus,
    tracking: Option[String] = None,
    extra: String,
    uuid: String)

case class BOReturnedItem(
    quantity: Int,
    refunded: Long,
    totalRefunded: Long,
    @JsonScalaEnumeration(classOf[ReturnedItemStatusRef]) status: ReturnedItemStatus.ReturnedItemStatus,
    boReturns: List[BOReturn],
    uuid: String)

case class BOReturn(
    motivation: Option[String],
    @JsonScalaEnumeration(classOf[ReturnStatusRef]) status: ReturnStatus.ReturnStatus,
    uuid: String,
    dateCreated: DateTime)
