...
import com.mogobiz.pay.common.{Cart => CartPay, CartItem => CartItemPay, Coupon => CouponPay, RegisteredCartItem => RegisteredCartItemPay, Shipping => ShippingPay, ShopCart => ShopCartPay, _}
...
class CartHandler extends StrictLogging {
  ...
  def queryCartItemAdd(storeCode: String,
    ...
    val transactionalBloc = { implicit session: DBSession =>
      ...
      if (cmd.dateTime.isEmpty && !ProductCalendar.NO_DATE.equals(product.calendarType))
  ...
  def queryCartPaymentPrepare(storeCode: String,
    ...
    val updatedCartPrice = runInTransaction(transactionalBloc, { cartPrice: StoreCartWithPrices =>
  ...
  def shippingPrices(cart: CartPay, accountId: String): ShippingCart = {
    ...
    ShippingCart(externalShippingPrices + (MogopayConstant.SHOP_MOGOBIZ -> addressAndList._2))
  }
  ...
  protected def createBOCart(cartWithChanges: CartWithPricesAndChanges,
    ...
    val newShopCartAndBOList = cart.shopCarts.map { shopCart =>
      ...
      val newCartItemAndBOList = shopCart.cartItems.map { cartItem =>
        ...
        val newRegisteredCartItemAndBOList = cartItem.registeredCartItems.map { registeredCartItem =>
  ...
  protected def initCart(storeCode: String,
    ...
    def prepareCart(cart: StoreCart) = {
      if (removeUnsalableItem) removeAllUnsellableItemsFromCart(cart, country, state) else cart
    }
    ...
    def getOrCreateStoreCart(cart: Option[StoreCart]): StoreCart = {
      cart match {
        ...
        case None =>
          val c = StoreCart(storeCode = storeCode, dataUuid = uuid, userUuid = currentAccountId)
          StoreCartDao.save(c)
          c
      }
    }
  ...
  protected def cancelCart(cartAndChanges: CartWithChanges)(implicit session: DBSession): CartWithChanges = {
    val cart = cartAndChanges.cart
    cart.boCartUuid.map { boCartUuid =>
      val newBoCart = boCartHandler.find(cart.storeCode, boCartUuid).map { boCart =>
  ...
  protected def validateCart(cart: StoreCart)(implicit session: DBSession): CartWithChanges = {
    if (!cart.validate) {
      val changes = CartChanges(stockChanges = validateShopCarts(cart.shopCarts))
  ...
  protected def validateShopCarts(shopCarts: List[StoreShopCart])(implicit session: DBSession): List[StockChange] = {
    if (shopCarts.isEmpty) Nil
    else validateCartItems(shopCarts.head.cartItems) ++ validateShopCarts(shopCarts.tail)
  }
  ...
  protected def invalidateCart(cart: StoreCart)(implicit session: DBSession): CartWithChanges = {
    if (cart.validate) {
      val changes = CartChanges(stockChanges = invalidateShopCarts(cart.shopCarts))
      CartWithChanges(cart.copy(validate = false, validateUuid = None), changes)
    } else CartWithChanges(cart, CartChanges())
  }

  protected def invalidateShopCarts(shopCarts: List[StoreShopCart])(implicit session: DBSession): List[StockChange] = {
    if (shopCarts.isEmpty) Nil
    else invalidateCartItems(shopCarts.head.cartItems) ++ invalidateShopCarts(shopCarts.tail)
  }
  ...
  protected def computePricesForShopCarts(storeCode: String,
  ...
  protected def computeDataForCoupon(storeCode: String, shopCart: StoreShopCart) : List[CouponWithData] = {
    ...
    } ::: CouponDao.findPromotionsThatOnlyApplyOnCart(storeCode).map { promotion =>
...
/*
  ...
  protected def applyReductionOnCartItem(storeCode: String,
                                         shopCart: StoreShopCart,
  ...
  protected def findBestReductionForCartItem(storeCode: String,
                                             cartItem: StoreCartItemWithPrices,
  ...
  protected def findSuggestionDiscount(storeCode: String, shopCart: StoreShopCart, productId: Long): List[String] = {
    ...
    val suggestions: List[Mogobiz.Suggestion] = SuggestionDao.getSuggestionsbyId(storeCode, productId)
  ...
  protected def renderCart(cart: StoreCartWithPrices, currency: Currency, locale: Locale): Map[String, Any] = {
  ...
  protected def renderTransactionCart(cart: StoreCartWithPrices,
    ...
    val reduction  = rateService.calculateAmount(mogobizShopCart.totalReduction, rate)
  ...
  /**
    ...
    */
  protected def renderCoupon(storeCode: String, couponWithData: CouponWithPrices, currency: Currency, locale: Locale) = {
  ...
  /**
    ...
    */
  protected def renderCartItem(item: StoreCartItemWithPrices, currency: Currency, locale: Locale) = {
    import org.json4s.jackson.JsonMethods._
