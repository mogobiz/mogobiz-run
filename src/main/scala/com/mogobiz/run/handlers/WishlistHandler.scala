...
class WishlistHandler {
  ...
  def runInTransaction[U, T](store: String,
                             wishlistListId: String,
                             owneremail: String,
                             call: (DBSession, BOWishList, WishlistList) => WishlistList,
                             success: WishlistList => U): U = {
    GlobalUtil.runInTransaction({
      implicit session: DBSession =>
        BOWishListDao
          .load(store, wishlistListId)
          .map { boWishList =>
            EsClient
              .load[WishlistList](esStore(store), wishlistListId, "wishlistlist")
              .map { wishlistList =>
  ...
  def addItem(store: String, wishlistListId: String, wishlistId: String, item: WishItem, owneremail: String): String = {
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          if (wishlist.items.exists { i: WishItem =>
                (i.product == item.product && i.sku == item.sku)
              })
            throw new DuplicateException(s"${item.name}")

          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(wishlists = wishlistList.wishlists
                  .filter(_.uuid != wishlistId) :+ wishlist.copy(items = wishlist.items :+ item, lastUpdated = now))
    ...
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          ...
          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(wishlists = wishlistList.wishlists
                  .filter(_.uuid != wishlistId) :+ wishlist.copy(brands = wishlist.brands :+ brand, lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
    ...
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          ...
          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(
              wishlists = wishlistList.wishlists.filter(_.uuid != wishlistId) :+ wishlist
                  .copy(categories = wishlist.categories :+ cat, lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
    ...
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(wishlists = wishlistList.wishlists
                  .filter(_.uuid != wishlistId) :+ wishlist.copy(ideas = wishlist.ideas :+ idea, lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
  ...
  def removeItem(store: String,
                 wishlistListId: String,
                 wishlistId: String,
                 itemUuid: String,
                 owneremail: String): Unit = {
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        .find(_.uuid == wishlistId)
        .map { wishlist =>
          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(
              wishlists = wishlistList.wishlists.filter(_.uuid != wishlist.uuid) :+ wishlist
                  .copy(items = wishlist.items.filter(_.uuid != itemUuid), lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
    ...
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          ...
          val newWishList = wishlistList.copy(
              wishlists = wishlistList.wishlists.filter(_.uuid != wishlist.uuid) :+ wishlist
                  .copy(ideas = wishlist.ideas.filter(_.uuid != ideaUuid), lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
    ...
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          ...
          val newWishList = wishlistList.copy(
              wishlists = wishlistList.wishlists.filter(_.uuid != wishlist.uuid) :+ wishlist
                  .copy(brands = wishlist.brands.filter(_.uuid != brandUuid), lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
  ...
  def removeCategory(store: String,
                     wishlistListId: String,
                     wishlistId: String,
                     catUuid: String,
                     owneremail: String): Unit = {
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          ...
          val newWishList = wishlistList.copy(
              wishlists = wishlistList.wishlists.filter(_.uuid != wishlist.uuid) :+ wishlist
                  .copy(categories = wishlist.categories.filter(_.uuid != catUuid), lastUpdated = now))
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
  ...
  def addWishlist(store: String, wishlistListId: String, wishlist: Wishlist, owneremail: String): String = {
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      ...
      val wishlistsWithUpdatedDefault =
        wishlistList.wishlists.map(ws => ws.copy(default = (!useNewWishListAsDefault && ws.default)))
      val now = Calendar.getInstance().getTime
      val newWishList = wishlistList.copy(
          wishlists = wishlistsWithUpdatedDefault :+ wishlist
              .copy(token = newUUID, default = useNewWishListAsDefault, dateCreated = now, lastUpdated = now))
      BOWishListDao.save(boWishList, newWishList)(session)
      newWishList
    }
    val successBloc = { newWishList: WishlistList =>
      ...
      wishlistList.wishlists
        ...
        .map { wishlist =>
          def setFirstAsDefault(list: List[Wishlist]): List[Wishlist] = {
            if (list.isEmpty || !wishlist.default)
  ...
  def setDefaultWishlist(store: String, wishlistListId: String, wishlistId: String, owneremail: String): Unit = {
    val transactionalBloc = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
      wishlistList.wishlists
        ...
        .map { wishlist =>
          val now = Calendar.getInstance().getTime
          val newWishList = wishlistList.copy(wishlists = wishlistList.wishlists.map(ws =>
                    ws.copy(default = (ws.uuid == wishlist.uuid))), lastUpdated = now)
          BOWishListDao.save(boWishList, newWishList)(session)
          newWishList
        }
        .getOrElse(throw NotFoundException(s"Invalid wishlist uuid $wishlistId"))
  ...
  def getWishlistList(store: String, owner_email: String, wishlistUuid: Option[String] = None): WishlistList = {
    ...
    existingWishlistUuid.map { wishlistListId =>
      val transactionalBlock = { (session: DBSession, boWishList: BOWishList, wishlistList: WishlistList) =>
        wishlistList
      }
      ...
      wishlistList.wishlists
        ...
        .map { wishlist =>
          if (wishlist.visibility == WishlistVisibility.PRIVATE) {
            val newWishlists = wishlistList.wishlists.filter(_.uuid != wishlistId) :+ wishlist.copy(
...
object RunApp extends App {
  ...
  service
    .addItem(Store, wll.uuid, wluuid, WishItem(name = "My first item", product = "product-uuid"), "hayssam@saleh.fr")
  service
    .addItem(Store, wll.uuid, wluuid, WishItem(name = "My second item", product = "product-uuid"), "hayssam@saleh.fr")
  service.setOwnerInfo(
      Store,
      wll.uuid,
      WishlistOwner("hayssam@saleh.fr", Some("Hayssam Saleh"), Some(15), Some(9), Some("Qui suis-je ?")))
