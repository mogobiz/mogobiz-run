...
case class CartChanges(stockChanges: List[StockChange] = Nil, boCartChange: Option[BOCart] = None, deletedBOCart: Option[BOCart] = None, saleChanges: List[SaleChange] = Nil)
...
case class StoreCart(storeCode: String,
    ...
    @JsonSerialize(using = classOf[JodaDateTimeSerializer])@JsonDeserialize(using = classOf[JodaDateTimeDeserializer]) expireDate: DateTime = DateTime.now.plusSeconds(60 * Settings.Cart.Lifetime), // Date d'expiration du panier
