...
object Learning {
  def esPurchasePredictions(store: String, segment: Option[String]): String =
    s"${store}_predictions_purchase" + segment.map("_" + _).getOrElse("").toLowerCase()

  def esViewPredictions(store: String, segment: Option[String]): String =
    s"${store}_predictions_view" + segment.map("_" + _).getOrElse("").toLowerCase()

  def esLearningStorePattern(store: String): String = s"${store}_learning_.*"

  def esFISStore(store: String, segment: Option[String]): String =
    s"${store}_fis" + segment.map("_" + _).getOrElse("").toLowerCase()
  ...
  case class UserItemAction(uuid: String,
    ...
    def toMap: Map[String, String] = {
      Map("uuid"        -> uuid,
          ...
          "dateCreated" -> dateCreated.getTime.toString,
          "lastUpdated" -> lastUpdated.getTime.toString)
    }
  }
  ...
  case class CartCombination(uuid: String,
