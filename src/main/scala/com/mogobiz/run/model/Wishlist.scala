...
case class WishlistOwner(email: String,
                         name: Option[String] = None,
                         dayOfBirth: Option[Int] = None,
                         monthOfBirth: Option[Int] = None,
                         description: Option[String] = None)

case class Wishlist(uuid: String = newUUID,
                    name: String,
                    @JsonScalaEnumeration(classOf[WishlistVisibilityRef]) visibility: WishlistVisibility =
...
case class AddWishlistCommand(name: String,
                              visibility: WishlistVisibility = WishlistVisibility.PRIVATE,
                              defaultIndicator: Boolean = false,
                              owner_email: String,
...
object BOWishListDao extends SQLSyntaxSupport[BOWishList] with BoService {
  ...
  def save(wishlist: BOWishList, wishlistExtra: WishlistList)(implicit session: DBSession): BOWishList = {
    ...
    withSQL {
      update(BOWishListDao)
        ...
        .where
        .eq(BOWishListDao.column.id, updatedWishlist.id)
    }.update.apply()
  ...
  def create(company: String, wishlistExtra: WishlistList)(implicit session: DBSession): BOWishList = {
    ...
    val newWishlist =
      new BOWishList(newId(), wishlistExtra.uuid, company, getExtra(wishlistExtra), DateTime.now, DateTime.now)
